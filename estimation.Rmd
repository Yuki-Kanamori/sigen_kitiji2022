...
dir = "/Users/Yuki/Dropbox/業務/キチジ太平洋北部/SA2022/"
fileEncoding = "CP932"
...
{r fig2}
図2の作成（R3年度にRコードが配布され，コードを使って作成せよと指示があったため）
-> 毎年同じ図なのでコメントアウト．両軸グラフだから不安定になりがち
...
```{r prefecture, echo=F, message=FALSE, results='hide', warning=F}
...
```{r figA3-4, echo=F, warning=F, message=F, results=F}
...
# sum(okisoko$漁獲量の合計)/1000
c20_oki %>% select(year, sum) %>% dplyr::rename(catch_ton = sum)
```
