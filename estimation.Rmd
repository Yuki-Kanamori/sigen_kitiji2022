...
for(i in 1:length(sheets)){
  ...
  # その後，調査で得た体長データ（年齢査定していないもの．南北の海域ごとに海底面積で引き延ばされたもの）を掛けると，年齢別体長別尾数が算出される
...
# ns_rec = ns_rec %>% dplyr::mutate(weight = 1.867*10^(-5)*((ns_rec$size_class+0.5)*10)^(3.068))
