...
# SS用のデータ
ssdata = catch_t3 %>% gather(key = method, value = catch_t, 2:ncol(catch_t3)) %>% dplyr::group_by(method) %>% dplyr::summarize(catch_t = sum(catch_t))
...
write.csv(ssdata, "okisoko_for_SS.csv", fileEncoding = fileEncoding, row.names=F)
